...
export default function HeadlineMetricSlopeCard({
  d,
}: {
  d: HeadlineMetricDatum;
}) {
  ...
  const delta =
    d.previous === 0 ? 0 : ((d.current - d.previous) / d.previous) * 100;
    ...
    <div className="panel metric-card" aria-label={d.label} ref={ref}>
      <div className="card-head">
        <div className="hstack" style={{ gap: 8 }}>
          <strong className="card-title" title={d.label}>
            {d.label}
          </strong>
          <button
            className="icon-btn"
